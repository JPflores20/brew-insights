    ...
    try {
        ...
        b1360.forEach(r => {
            for (let i = 1; i <= 24; i++) {
                ...
                const name = String(r[`NAME_DFM${i}`]).toLowerCase();
                if (unit === 'kg' && !name.includes('malta')) {
                    kgRecords.push({
                        teilanl: r.TEILANL,
                        gop: r.GOP_BEZ,
                        dfm: `DFM${i}`,
        ...
        console.log("Non-Malta KG records for batch 1360:", JSON.stringify(kgRecords, null, 2));
    } catch (e) {
