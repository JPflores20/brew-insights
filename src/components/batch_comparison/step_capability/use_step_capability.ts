...
export function useStepCapability(data: BatchRecord[]) {
    ...
    data.forEach(d => {
      if (selectedMachines.includes(d.TEILANL_GRUPO)) {
        d.parameters.forEach(p => {
          if (selectedSteps.includes(p.stepName)) params.add(p.name);
        });
      }
    });
  ...
  const analysisValues = useMemo(() => {
    ...
    return data
      ...
      .flatMap((d) => d.parameters.filter(p => selectedSteps.includes(p.stepName) && selectedParams.includes(p.name)).map(p => Number(p.value)))
  ...
  return {
    ...
    lei, handleLeiChange,
    les, handleLesChange,
    uniqueRecipes, uniqueMachines, uniqueSteps, uniqueParams,
