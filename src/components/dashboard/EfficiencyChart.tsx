...
export function EfficiencyChart({ data, className, titleClassName }: EfficiencyChartProps) {
  // Definimos el orden lógico del proceso
  const processOrder = [
    "molienda",
    "grits",
    "cocedor",
  ...
  const getSortIndex = (name: string) => {
    ...
    const index = processOrder.findIndex(key => lowerName.includes(key));
    return index === -1 ? 999 : index;
  };

  // 1. Preparamos los datos base ORDENADOS
  const allChartData = useMemo(() => {
      return getAveragesByMachine(data)
        .sort((a, b) => getSortIndex(a.machine) - getSortIndex(b.machine))
        .map(item => ({
          machine: item.machine, // Guardamos nombre completo para filtro
          displayMachine: item.machine.replace(/(\d)/, '\n$1'), // Nombre para display
